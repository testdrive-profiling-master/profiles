...
set SIM_DEFINITION_EXTRA =

if "%SIM_WAVE_MODE%"=="File output" (
	echo 	Waveform configuration : File output mode.
	if "%SIM_MULTITHREADING%"=="Disable" (
		set SIM_DEFINITION_EXTRA=%SIM_DEFINITION_EXTRA% --trace-fst -trace-max-width 4096
	) else (
		set SIM_DEFINITION_EXTRA=%SIM_DEFINITION_EXTRA% --trace-fst-thread -trace-max-width 4096
	)
) else if "%SIM_WAVE_MODE%"=="Interactive" (
	echo 	Waveform configuration : Interactive[with 'GTKWave'] mode.
	set SIM_DEFINITION_EXTRA=%SIM_DEFINITION_EXTRA% --trace -trace-max-width 4096
) else (
...
if "%SIM_MULTITHREADING%"=="Enable(HDL)" (
	...
	set SIM_DEFINITION_EXTRA=%SIM_DEFINITION_EXTRA% --threads %thread_count% --threads-dpi none
) else if "%SIM_MULTITHREADING%"=="Enable(HDL+DPI)" (
	echo 	Multithreading : Enable[HDL+DPI]
	set SIM_DEFINITION_EXTRA=%SIM_DEFINITION_EXTRA% --threads %thread_count% --threads-dpi all
) else (
	echo 	Multithreading : Disable
)

verilator %SIM_TOP_FILE% %1 %2 %3 %4 %SIM_DEFINITION% %SIM_DEFINITION_EXTRA% -sv -cc --autoflush --compiler gcc --output-split 100000 -Mdir ../SubSystems/Simulation/verilator -prefix SimTop --top-module top
ENDLOCAL